...
class ToolsCategory(Document):
    ...
    def validate_account_currency(self):
        account_types = [
                'iat_account', 'accumulated_depreciation_account', 'depreciation_expense_account', 'capital_work_in_progress_account'
        ]
        invalid_accounts = []
    ...
    def validate_account_types(self):
        account_type_map = {
                'iat_account': {'account_type': ['Instrument And Tool']},
...
@frappe.whitelist()
def get_tools_category_account(fieldname, item=None, iat=None, account=None, tool_category = None, company = None):
    if item and frappe.db.get_value("Item", item, "is_fixed_asset"):
        tool_category = frappe.db.get_value("Item", item, ["tools_category"])

    elif not tool_category or not company:
        if account:
            if frappe.db.get_value("Account", account, "account_type") != "Instrument And Tool":
                account = None

        if not account:
            iat_details = frappe.db.get_value("Instrument", iat, ["tools_category", "company"])
            tool_category, company = iat_details or [None, None]

    account = frappe.db.get_value("IaT Category Account",# lưu ý
            filters={"parent": tool_category, "company_name": company}, fieldname=fieldname)
