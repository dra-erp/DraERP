...
# For license information, please see license.txttools_category
...
class IaTMaintenance(Document):
    def validate(self):
        for task in self.get('iat_maintenance_tasks'):
    ...
    def sync_maintenance_tasks(self):
        ...
        for task in self.get('iat_maintenance_tasks'):
            ...
            update_maintenance_log(iat_maintenance = self.name, item_code = self.item_code, item_name = self.item_name, task = task)
        iat_maintenance_logs = frappe.get_all("IaT Maintenance Log", fields=["name"], filters = {"iat_maintenance": self.name,
                "task": ("not in", tasks_names)})
        if iat_maintenance_logs:
            for iat_maintenance_log in iat_maintenance_logs:
                maintenance_log = frappe.get_doc('IaT Maintenance Log', iat_maintenance_log.name)
                maintenance_log.db_set('maintenance_status', 'Cancelled')

@frappe.whitelist()
def assign_tasks(iat_maintenance_name, assign_to_member, maintenance_task, next_due_date):
    team_member = frappe.db.get_value('User', assign_to_member, "email")
    args = {
            'doctype' : 'IaT Maintenance',
            'assign_to' : [team_member],
            'name' : iat_maintenance_name,
...
def update_maintenance_log(iat_maintenance, item_code, item_name, task):
    iat_maintenance_log = frappe.get_value("IaT Maintenance Log", {"iat_maintenance": iat_maintenance,
            "task": task.name, "maintenance_status": ('in',['Planned','Overdue'])})

    if not iat_maintenance_log:
        iat_maintenance_log = frappe.get_doc({
                "doctype": "IaT Maintenance Log",
                "iat_maintenance": iat_maintenance,
                "iat_name": iat_maintenance,
                "item_code": item_code,
                "item_name": item_name,
                "task": task.name,
        ...
        iat_maintenance_log.insert()
    else:
        maintenance_log = frappe.get_doc('IaT Maintenance Log', iat_maintenance_log)
