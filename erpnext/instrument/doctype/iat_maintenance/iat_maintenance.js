...
frappe.ui.form.on('IaT Maintenance', {
	setup: (frm) => {
		frm.set_query("assign_to", "iat_maintenance_tasks", function (doc) {
			return {
				query: "erpnext.instrument.doctype.iat_maintenance.iat_maintenance.get_team_members",
				filters: {
	...
	make_dashboard: (frm) => {
		if (!frm.is_new()) {
			frappe.call({
				method: 'erpnext.instrument.doctype.iat_maintenance.iat_maintenance.get_maintenance_log',
				args: { iat_name: frm.doc.iat_name },
				callback: (r) => {
					if (!r.message) {
						return;
					}
					...
					(r.message || []).forEach(function (d) {
						$(`<div class='row' style='margin-bottom: 10px;'>
							<div class='col-sm-3 small'>
								<a onclick="frappe.set_route('List', 'IaT Maintenance Log',
									{'iat_name': '${d.iat_name}','maintenance_status': '${d.maintenance_status}' });">
...
frappe.ui.form.on('IaT Maintenance Task', {
...
var get_next_due_date = function (frm, cdt, cdn) {
	...
	if (d.start_date && d.periodicity) {
		return frappe.call({
			method: 'erpnext.instrument.doctype.iat_maintenance.iat_maintenance.calculate_next_due_date',
			args: {
