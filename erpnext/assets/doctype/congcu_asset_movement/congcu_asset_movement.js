...
frappe.ui.form.on('CongCu_Asset_Movement', {
	setup: (frm) => {
		...
		frm.set_query("congcu_asset", "assets", () => {
			return {
				filters: {
					status: ["not in", ["Draft"]]
				}
...
frappe.ui.form.on('Asset Movement Item', {
	congcu_asset: function(frm, cdt, cdn) {
		// on manual entry of an asset auto sets their source location / employee
		const asset_name = locals[cdt][cdn].congcu_asset;
		if (asset_name){
			frappe.db.get_doc('CongCu_Asset', asset_name).then((asset_doc) => {
