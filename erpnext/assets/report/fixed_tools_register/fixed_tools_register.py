...
def execute(filters=None):
    ...
    chart = prepare_chart_data(data, filters) if filters.get("group_by") not in ("Tools Category", "Location") else {} 
...
def get_conditions(filters):
    ...
    if filters.get('tools_category'):
        conditions["tools_category"] = filters.get('tools_category')
...
def prepare_chart_data(data, filters):
    ...
    return {
            "data" : {
                    ...
                    "datasets": [
                            {'name': _('Tools Value'), 'values': [d.get("asset_value") for d in labels_values_map.values()]},
